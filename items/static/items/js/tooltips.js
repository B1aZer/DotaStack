	...
	$.each( [ 'item', 'ability', 'hero' ], 
		function( i, ttType )
		{
			...
			if ( icons.length )
			{
				...
				// .live doesn't work against our subset collection, add hover handler to each element we found
				if ( g_bSearchIMGTags )
				{
					icons.each(
						function( i )
						{
							$(this).hover( 
								...
								function( e )
								{
									g_tooltipDelayCount	= setTimeout( ShowTooltip, getTooltipHoverDelay(), ttType, e.currentTarget, true );
								},
								// Mouse out
								function( e )
								{
									clearTimeout( g_tooltipDelayCount );
									ShowTooltip( ttType, e.currentTarget, false );
								}
							);
						}
					);
	...
	$('<link>').attr(
		{
			rel:"stylesheet",
			type:"text/css",
			href:"http://www.dota2.com/public/css/tooltips.css?v=1764963835"
		}
	).appendTo('head');
...
function LoadHeropediaData( rgFeeds )
{
	...
	URL = URL + 'jsfeed/heropediadata?feeds='+strFeeds+'&v=23105701764963835&l=russian';
	$.ajax(
		{
			type:'GET',
			cache:true,
...
function BuildItemTooltipHTML( itemName )
{
	...
	if ( iData.mc || iData.cd )
	{
		...
		if ( iData.mc )
		{
			strHTML += '<div class="mana"><img alt="Расход маны" title="Расход маны" class="manaImg" src="http://cdn.dota2.com/apps/dota2/images/tooltips/mana.png" width="22" height="22" border="0" />'+iData.mc+'</div>';
		}
		if ( iData.cd )
		{
...
function BuildAbilityTooltipHTML( abilityName )
{
	...
	if ( aData !== undefined ) {
		...
		strHTML += '<div class="abilityName">'+"Бонус к атрибутам"+'</div>';
		strHTML += '<div class="abilityHR1"></div>';
		strHTML += '<div class="abilityDesc">'+"Бонусы к атрибутам влияют на все три характеристики героя: силу, ловкость и интеллект."+'</div>';
	}
...
function ShowTooltip( iconType, iconElement, bShow )
{
	...
	else
	{
		...
		{
			tooltipElement = $(document.createElement('div')).appendTo('body');
			tooltipElement.attr( 'id', 'iconTooltip' );
			tooltipElement.addClass('iconTooltip');
		}
		...
		tooltipElement.css(
			{
				...
				left:iconElementPos.left + ( bAnchorToIconLeft ? (tooltipElement.outerWidth()+iArrowWidth-6)*-1 : iconElement.outerWidth()+iArrowWidth )
			}
		);
		...
		{
			$('<img class="iconTooltipArrow" src="http://cdn.dota2.com/apps/dota2/images/tooltips/tt_arrow_left'+arrowImgSrcSuffix+'.png" width="29" height="34" border="0" />').css(
				{
					left:(iArrowWidth-0)*-1,
		...
		{
			$('<img class="iconTooltipArrow" src="http://cdn.dota2.com/apps/dota2/images/tooltips/tt_arrow_right'+arrowImgSrcSuffix+'.png" width="29" height="34" border="0" />').css(
				{
